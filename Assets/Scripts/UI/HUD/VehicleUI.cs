...
namespace Vehicle
{
    ...
    public class VehicleUI : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (speedometerFill != null)
            {
                // 使用车辆实际的最大速度或者预设的maxSpeedForGauge，取较大值
                float actualMaxSpeed = Mathf.Max(vehicleController.GetMaxSpeed(), maxSpeedForGauge);
                speedometerFill.fillAmount = Mathf.Clamp01(currentSpeed / actualMaxSpeed);
            }
