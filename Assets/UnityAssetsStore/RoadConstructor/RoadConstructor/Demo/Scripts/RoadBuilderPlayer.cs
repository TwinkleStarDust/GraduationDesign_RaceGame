...
namespace PampelGames.RoadConstructor.Demo
{
    ...
    public class RoadBuilderPlayer : RoadBuilderBase
    {
        ...
        private void Update()
        {
            ...
            SetPointerDemolishActive(demolishActive);
            ...
            if (moveActive)
            {
                ...
                pointerPosition = SnapPointer(hit.point, hit.normal);
                ...
                else if (moveStatus == MoveStatus.Move)
                {
                    if (Input.GetKeyDown(KeyCode.Mouse0))
                    {
                        var resultMove = roadConstructor.MoveIntersection(movePosition, GetDefaultRadius(), hit.point);
                    ...
                    {
                        var resultDisplayMove = roadConstructor.DisplayMoveIntersection(movePosition, GetDefaultRadius(), hit.point);    
                    }
                }
            }
            ...
            if (Input.GetKeyDown(KeyCode.Mouse0)) // Actual construction of the roads & intersections.
            {
                ...
                else result = ConstructRamp(pointerPosition, roadSettings);
            }
            else // Creating display objects, only used for visual representation.
            {
                ...
                else result = DisplayRamp(pointerPosition, roadSettings);
            }
