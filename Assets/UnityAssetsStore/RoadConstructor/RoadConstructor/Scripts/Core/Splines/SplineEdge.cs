...
namespace PampelGames.RoadConstructor
{
    ...
    public class SplineEdgeEditor : PGIDuplicable
    {
        ...
        public Vector2 positionX = new(0f, 1f);
    ...
    internal static class SplineEdgeUtility
    {
        ...
        private static SplineEdge[] CreateSplineEdges(RoadDescr roadDescr, SplineEdgeEditor splineEdgeEditor, float baseRoadHeight)
        {
            ...
            var splineEdges = new List<SplineEdge>
            {
                new()
                {
                    position = new float2(positionX, correctedHeight), uvX = splineEdgeEditor.uvX.x, normalRotation = 0f
                },
                new()
                {
                    position = new float2(positionY, correctedHeight), uvX = splineEdgeEditor.uvX.y, normalRotation = 0f
                }
            };
            ...
            if (!Mathf.Approximately(height, 0f))
            {
                ...
                {
                    position = new float2(positionX, 0f), uvX = splineEdgeEditor.uvX.x + uvRatio, normalRotation = -90f
                });

                splineEdges.Add(new SplineEdge
                {
                    position = new float2(positionY, 0f), uvX = splineEdgeEditor.uvX.y - uvRatio, normalRotation = 90f
                });
            }
