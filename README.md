...
- **空中桶滚功能增强**
  ...
  - 将力矩应用模式从`ForceMode.Force`改为`ForceMode.Impulse`，使桶滚效果更加明显
  - 大幅增加空中翻滚力矩(30000→100000)和俯仰力矩(20000→80000)，提升控制灵敏度
  - 提高最大角速度限制(5.0→8.0)，允许车辆达到更高的旋转速度
  - 减弱空中姿态稳定效果，使车辆在空中更容易进行翻滚动作
  - 增加空中控制时的向上力，使车辆在空中保持更好的高度
...
- **空中控制使用技巧**
  ...
  - 松开按键后，车辆会逐渐停止旋转，但不会立即停止
  - 高速旋转时，系统会限制最大角速度，防止旋转过快导致失控

- **空中控制参数调整建议**
  - 如果桶滚效果不明显：增加`airRollTorque`值或减小`maxAngularSpeed`限制
  - 如果桶滚过于敏感：减小`airRollTorque`值或增加阻尼效果
  - 如果车辆在空中下落过快：增加空中控制时的向上力
  - 如果车辆在空中难以保持平衡：增加空中姿态稳定效果的强度
